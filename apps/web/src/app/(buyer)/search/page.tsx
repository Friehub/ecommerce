        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            <input 
              type="number" 
              placeholder="MIN" 
              className="w-full bg-surface-container-low/30 border-2 border-surface-container-low rounded-xl px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-on-surface focus:border-jumia-orange outline-none transition-all placeholder:font-normal placeholder:text-on-surface-variant/50" 
              value={minPrice}
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
            <label className="flex items-center gap-4 cursor-pointer group">
              <input 
                ...
                className="w-4 h-4 rounded-full border-2 border-outline-variant text-jumia-orange focus:ring-primary-container bg-surface-container-low" 
              />
              <span className="group-hover:text-jumia-orange font-semibold text-on-surface-variant uppercase text-[10px] tracking-widest transition-colors opacity-80">All Nodes</span>
            </label>
            {brands.map((b: any) => (
              <label key={b.id} className="flex items-center gap-4 cursor-pointer group">
                <input 
                  ...
                  className="w-4 h-4 rounded-full border-2 border-outline-variant text-jumia-orange focus:ring-primary-container bg-surface-container-low" 
                />
                <span className="group-hover:text-jumia-orange font-semibold text-on-surface-variant uppercase text-[10px] tracking-widest transition-colors opacity-80">{b.name}</span>
              </label>
            ))}
          </div>
        </div>
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <aside className="hidden lg:block w-80 flex-shrink-0">
          <div className="bg-surface-container-lowest rounded shadow-soft border border-surface-container-low overflow-hidden sticky top-24">
        ...
        {isFilterOpen && (
          <div className="fixed inset-0 z-[100] lg:hidden">
            <div className="absolute inset-0 bg-jumia-orange/60 backdrop-blur-md" onClick={() => setIsFilterOpen(false)} />
            <div className="absolute bottom-0 left-0 right-0 bg-surface-container-lowest rounded-t-[48px] p-10 animate-in slide-in-from-bottom duration-500 shadow-2xl border-t-8 border-surface-container-low">
              ...
              <div className="flex items-center justify-between mb-10">
                <h3 className="text-2xl font-semibold text-on-surface uppercase tracking-tighter">Filter Console</h3>
        ...
        <main className="flex-1">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-8 bg-surface-container-lowest p-8 rounded border border-surface-container-low shadow-soft">
            <div>
            ...
            <div className="flex items-center gap-4 text-sm font-semibold bg-surface-container-low/30 px-6 py-4 rounded-sm border-2 border-surface-container-low shadow-sm text-on-surface">
              <SortAsc size={18} className="text-jumia-orange" />
              <select 
                value={sortBy}
                onChange={(e) => {
                ...
                className="border-none bg-transparent font-semibold focus:ring-0 text-[10px] cursor-pointer outline-none text-on-surface uppercase tracking-widest appearance-none"
              >
            ...
            <div className="flex flex-wrap items-center gap-4 mb-10 animate-in fade-in slide-in-from-left-4 duration-700">
              ...
              {minPriceParam && (
                <button 
                  ...
                  className="bg-surface-container-low px-5 py-2.5 rounded-full border-2 border-surface-container-lowest flex items-center gap-3 text-[9px] font-semibold uppercase tracking-widest text-on-surface hover:border-jumia-orange transition-all shadow-sm"
                >
                  Min: ₦{parseInt(minPriceParam).toLocaleString()}
                  <X size={14} className="text-jumia-orange" />
                </button>
              )}
              {maxPriceParam && (
                <button 
              ...
              <button 
                ...
                className="text-[9px] font-semibold uppercase tracking-widest text-error hover:underline ml-4 italic"
              >
                Purge All Filters
              </button>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="bg-surface-container-lowest rounded-[64px] border-8 border-surface-container-low shadow-soft py-40 text-center px-10 animate-in fade-in zoom-in-95 duration-1000">
              <div className="w-28 h-28 bg-surface-container-low text-on-surface-variant/20 rounded flex items-center justify-center mx-auto mb-10 border border-surface-container-lowest shadow-2xl">
                <SearchIcon size={48} />
              </div>
