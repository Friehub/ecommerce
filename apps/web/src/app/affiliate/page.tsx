...
export default function AffiliatePortal() {
  ...
  if (!profile) {
    return (
      <div className="bg-background min-h-screen pb-20 selection:bg-jumia-orange/30">
        <div className="bg-jumia-orange text-white py-32 overflow-hidden relative border-b-8 border-jumia-orange/20">
          <div className="absolute top-0 right-0 -mr-20 -mt-20 w-[600px] h-[600px] bg-jumia-orange/10 rounded-full blur-[120px] animate-pulse" />
        ...
        <div className="container mx-auto px-6 max-w-6xl -mt-20 relative z-20 grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            { icon: <BarChart3 size={24} />, title: 'Live Notifications', desc: 'Monitor your network pulse with real-time click and conversion data.' }
          ].map((benefit, i) => (
            <div key={i} className="bg-surface-container-lowest p-10 rounded border border-surface-container-low shadow-soft hover:-translate-y-2 transition-all duration-500 animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${i * 100}ms` }}>
  ...
  return (
    <div className="bg-background min-h-screen pb-20 select-none animate-in fade-in duration-1000">
      {/* Header */}
      <div className="bg-jumia-orange pt-16 pb-32 border-b border-white/5 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
            <div className="animate-in slide-in-from-left-8 duration-700">
              <div className="flex items-center gap-4 mb-6">
                <div className="bg-jumia-orange/20 backdrop-blur-xl p-2.5 rounded-2xl border-2 border-jumia-orange/30">
                  <Activity size={24} className="text-jumia-orange" />
                </div>
                <h1 className="text-4xl font-semibold text-white uppercase tracking-tighter leading-none">Partner <span className="text-jumia-orange italic">Terminal.</span></h1>
              </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`h-12 rounded-xl border-2 flex items-center justify-center gap-2 text-[8px] font-semibold uppercase tracking-widest transition-all ${targetType === btn.type ? 'bg-jumia-orange text-white border-on-surface' : 'bg-surface-container-low/30 border-surface-container-low text-on-surface-variant hover:border-on-surface-variant'}`}
                      >
                        {btn.icon} {btn.label}
                      </button>
                    ))}
                ...
                {targetType !== 'HOME' && (
                  <div className="space-y-3">
                    ...
                    <input 
                      ...
                      className="w-full h-14 px-6 bg-surface-container-low/30 border-2 border-surface-container-low rounded-2xl focus:border-jumia-orange focus:ring-4 focus:ring-primary-container/5 text-xs font-bold text-on-surface outline-none transition-all placeholder:font-normal placeholder:text-on-surface-variant/50"
                    />
                  </div>
                )}
                <button 
              ...
              <div className="divide-y-4 divide-surface-container-low max-h-[600px] overflow-y-auto custom-scrollbar">
                {profile.links.map(link => (
                  <div key={link.id} className="p-10 flex items-center justify-between hover:bg-surface-container-low/20 transition-all duration-500 group">
                    <div className="space-y-3">
                      <div className="flex items-center gap-4">
                        <p className="text-lg font-semibold text-on-surface uppercase tracking-tighter group-hover:text-jumia-orange transition-colors">taas.link/{link.slug}</p>
                        <div className="flex items-center gap-2">
                          <button 
                            onClick={() => copyToClipboard(`https://friehub.cloud/r/${link.slug}`, link.id)}
                            className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${copiedId === link.id ? 'bg-success text-white' : 'bg-surface-container-low text-on-surface-variant hover:bg-jumia-orange hover:text-white'}`}
                          >
                          ...
                          <a 
                            ...
                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-surface-container-low text-on-surface-variant hover:bg-jumia-orange-dark hover:text-white transition-all"
                          >
                    ...
                    <div className="text-right">
                      <p className="text-3xl font-semibold text-on-surface tracking-tighter leading-none group-hover:scale-110 transition-transform origin-right duration-500">{(link as any)._count?.clicks || 0}</p>
                      <p className="text-[9px] font-semibold text-on-surface-variant/30 uppercase tracking-widest mt-2">Notifications Pulses</p>
                    </div>
                  </div>
                ))}
                        ...
                        {commissions.map(comm => (
                          <tr key={comm.id} className="hover:bg-surface-container-low/10 transition-colors group">
                            <td className="px-10 py-8">
                              <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border-2 text-[8px] font-semibold uppercase tracking-widest ${
                            ...
                            <td className="px-10 py-8 text-[10px] font-semibold text-on-surface uppercase tracking-tight">#{comm.orderId.slice(-12).toUpperCase()}</td>
                            <td className="px-10 py-8 text-[10px] font-semibold text-on-surface-variant/40 uppercase tracking-widest italic">{new Date(comm.createdAt).toLocaleDateString()}</td>
                            <td className="px-10 py-8 text-right text-base font-semibold text-on-surface tracking-tighter group-hover:text-success transition-colors">₦{Number(comm.amount).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    <div className="px-10 py-8 border-t-4 border-surface-container-low flex items-center justify-between bg-surface-container-low/5">
                      ...
                      <div className="flex items-center gap-4">
                        <button
                          ...
                          disabled={page === 0}
                          className="w-12 h-12 flex items-center justify-center bg-surface-container-low rounded-xl text-on-surface-variant hover:bg-jumia-orange hover:text-white transition-all disabled:opacity-20"
                        >
                          <ChevronLeft size={18} />
                        </button>
                        <button
