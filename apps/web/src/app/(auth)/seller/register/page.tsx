...
import { Upload, FileText, CheckSquare, ArrowRight, UserPlus, Loader2, ChevronLeft } from 'lucide-react';
...
export default function SellerRegistrationKYCPage() {
  ...
  const onboardSeller = api.iam.onboardSeller.useMutation();
  const uploadDocMutation = api.iam.uploadDocument.useMutation();

  // Prepopulate email and phone from session if logged in
  useEffect(() => {
    if (session?.user) {
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen bg-surface-container-lowest flex items-center justify-center">
        <div className="animate-spin rounded-full h-10 w-10 border-2 border-jumia-orange border-t-transparent" />
      </div>
    );
  }
  ...
  if (status === 'unauthenticated') {
    return (
      <div className="min-h-screen bg-surface-container-lowest flex flex-col justify-center items-center py-20 px-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-[500px] bg-surface rounded shadow-soft border border-outline-variant/10 relative z-10 overflow-hidden text-center p-10 md:p-12">
          <div className="w-20 h-20 bg-jumia-orange/10 rounded-3xl flex items-center justify-center mx-auto mb-6 border border-jumia-orange/20 shadow-md">
            <UserPlus size={40} className="text-jumia-orange" />
          </div>
          <h1 className="text-3xl font-semibold text-on-surface uppercase tracking-tight mb-4">Partner registration</h1>
          <p className="text-on-surface-variant text-sm mb-8 leading-relaxed">
            To register as a merchant and sell your products on Jumia, you must first log in or create a shopper account.
          </p>
          
          <div className="space-y-4">
            <Link
              href="/login?callbackUrl=/seller/register"
              className="w-full h-16 bg-jumia-orange text-white rounded-2xl font-semibold uppercase tracking-widest shadow-lg shadow-primary-container/20 hover:shadow-2xl hover:translate-y-[-4px] active:translate-y-[1px] duration-300 transition-all flex items-center justify-center gap-4 cursor-pointer"
            >
              Sign In to Jumia
            </Link>
            
            <Link
              href="/register?callbackUrl=/seller/register"
              className="w-full h-16 bg-surface-container-low text-on-surface border-2 border-outline-variant/10 rounded-2xl font-semibold uppercase tracking-widest hover:bg-surface-container-high duration-300 transition-all flex items-center justify-center gap-4 cursor-pointer"
            >
              Create Account
            </Link>
          </div>
  ...
  const handleFinalize = async (e: React.FormEvent) => {
    ...
    if (!businessName) {
      toast({
        title: 'Error',
        description: 'Please enter your store business name',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      // 1. Create seller record and set User role to SELLER in backend
      await onboardSeller.mutateAsync({ businessName });
      ...
      if (cacDoc) {
        const res = await fetch('/api/media/upload', {
          ...
          body: JSON.stringify({ fileName: cacDoc.name, contentType: cacDoc.type }),
        });

        if (!res.ok) {
          throw new Error('Failed to retrieve secure upload endpoint from media service.');
        }

        const { uploadUrl, publicUrl } = await res.json();

        // Put file blob directly to S3
        const putRes = await fetch(uploadUrl, {
          method: 'PUT',
          body: cacDoc,
          headers: { 'Content-Type': cacDoc.type },
        });

        if (!putRes.ok) {
          throw new Error('Failed to synchronize KYC document with storage bucket.');
        }

        // Upload verification document to database
      ...
      // 3. Update JWT session cookie dynamically
      await update({ role: 'SELLER' });

      toast({
        title: 'Onboarding Complete',
        description: 'Your shop has been successfully registered! Redirecting to Merchant Hub...',
      });
      ...
      toast({
        ...
        description: err.message || 'An error occurred during partner registration.',
        variant: 'destructive',
      });
    } finally {
          ...
          {step === 1 ? (
            ...
            <form onSubmit={handleFinalize} className="space-y-8">
              ...
              <button
                type="button"
                onClick={() => setStep(1)}
                className="w-full text-center text-[10px] text-on-surface-variant font-semibold uppercase hover:text-jumia-orange transition-all cursor-pointer mt-4 opacity-40 flex items-center justify-center gap-2"
              >
