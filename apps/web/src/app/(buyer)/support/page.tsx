...
import { Send, ChevronRight, User, ShieldCheck, Activity, Zap } from 'lucide-react';
...
export default function ContactSupportPage() {
  ...
  const { data: ticket, isLoading: ticketLoading } = api.support.getActiveTicket.useQuery(undefined, {
    enabled: !!session,
  });

  // 2. Fetch ticket messages
  const { data: messages, refetch: refetchMessages } = api.support.getMessages.useQuery(
    { ticketId: ticket?.id ?? '' },
    { enabled: !!ticket?.id }
  );
  ...
  // 5. Short polling to fetch new agent messages (e.g. every 5 seconds)
  useEffect(() => {
    if (!ticket?.id) return;
    ...
    return (
      <div className="bg-background min-h-screen flex items-center justify-center select-none">
        <div className="flex flex-col items-center gap-6">
          <div className="w-12 h-12 border-4 border-surface-container-low border-t-jumia-orange rounded-full animate-spin" />
          <p className="text-[10px] font-black uppercase text-on-surface-variant/40 tracking-widest animate-pulse">Establishing Secure Uplink...</p>
        </div>
      </div>
    );
  }
  ...
  const messageList = messages || ticket?.messages || [];
    ...
    <div className="bg-background min-h-screen pb-24 select-none">
      <div className="container py-12 max-w-6xl mx-auto px-6">
        ...
        <div className="flex items-center gap-3 mb-12 font-semibold text-on-surface-variant text-[10px] uppercase ">
          ...
          <Link href="/help" className="hover:text-jumia-orange transition-colors">Help Center</Link>
          <ChevronRight size={14} className="opacity-30" />
          <span className="text-on-surface">Support Chat</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8 flex flex-col h-[700px] bg-surface-container-lowest rounded border border-surface-container-low shadow-soft overflow-hidden animate-in fade-in slide-in-from-right-8 duration-1000">
            ...
            <div className="p-8 bg-surface-container-low/30 border-b-2 border-surface-container-low flex items-center justify-between">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-semibold text-on-surface-variant/40 uppercase  mt-2 italic">Automated Support Center</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar bg-surface-container-low/5">
              {messageList.map((msg) => (
                <div key={msg.id} className={`flex gap-6 max-w-[85%] select-text animate-in slide-in-from-bottom-4 duration-500 ${msg.senderRole === 'USER' ? 'ml-auto flex-row-reverse' : 'mr-auto'}`}>
                  <div className={`w-10 h-10 rounded-[14px] flex items-center justify-center flex-shrink-0 border-2 transition-all ${msg.senderRole === 'USER' ? 'bg-jumia-orange/10 text-jumia-orange border-jumia-orange/20' : 'bg-surface-container-lowest border-surface-container-low text-on-surface-variant/20'}`}>
                    <User size={18} />
                  </div>
                  <div className={`p-6 rounded text-[11px] font-semibold uppercase tracking-widest leading-relaxed shadow-soft border-2 ${msg.senderRole === 'USER' ? 'bg-jumia-orange text-white border-on-surface rounded-tr-none' : 'bg-surface-container-lowest border-surface-container-low rounded-tl-none text-on-surface'}`}>
