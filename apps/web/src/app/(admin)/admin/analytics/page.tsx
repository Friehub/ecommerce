...
export default function AdminAnalyticsPage() {
 ...
 <div key={i} className="bg-surface-container-lowest p-8 rounded border border-surface-container-low shadow-soft group transition-all duration-700 hover:border-jumia-orange/20 hover:translate-x-2">
 <div className="flex items-center gap-4 mb-6">
 ...
 <div className="w-2 h-2 rounded-full bg-jumia-orange/30 group-hover/row:scale-150 transition-all duration-500" />
 <span className="text-xs font-semibold text-on-surface/60 uppercase tracking-widest">{point.day}</span>
 </div>
 <span className="text-sm font-semibold text-jumia-orange tracking-tight italic">₦ {point.value.toLocaleString()}</span>
 </div>
 ))
 ) : (
 ...
 <div className="h-full bg-jumia-orange transition-all duration-1000 ease-out shadow-[0_0_12px_rgba(246,139,30,0.4)]" style={{ width: `${Math.min(point.value * 5, 100)}%` }} />
 </div>
 <span className="text-xs font-semibold text-on-surface tracking-widest">{point.value}</span>
 </div>
 </div>
 ))
 ) : (
