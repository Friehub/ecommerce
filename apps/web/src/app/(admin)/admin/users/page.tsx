      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
        <div className="animate-in slide-in-from-left-8 duration-1000">
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-primary-container italic">User Access Protocol & Directory Registry</span>
          </div>
          <h1 className="text-5xl md:text-7xl font-black text-on-surface uppercase tracking-tighter leading-[0.85]">
            User <br />
            <span className="text-primary-container italic">Registry.</span>
          </h1>
        </div>

        <button className="bg-on-surface text-white px-10 py-5 rounded-2xl font-black text-[11px] hover:bg-primary-container transition-all duration-700 shadow-3xl uppercase tracking-[0.4em] italic flex items-center gap-4 group animate-in slide-in-from-right-8 duration-1000">
        ...
        <div className="p-10 border-b-4 border-surface-container-low flex flex-col lg:flex-row gap-8 items-center bg-surface-container-low/20">
          <div className="relative flex-1 w-full group/search">
            ...
            <input 
              ...
              className="w-full pl-16 pr-8 py-5 bg-surface-container-low border-4 border-surface-container-lowest rounded-[28px] focus:outline-none focus:border-primary-container/20 focus:ring-[20px] focus:ring-primary-container/5 text-xs font-black text-on-surface placeholder:text-on-surface-variant/10 shadow-inner transition-all duration-700 uppercase tracking-widest"
            />
          </div>
          <div className="flex gap-4 w-full lg:w-auto">
            <div className="relative flex-1 lg:flex-none">
              <Filter className="absolute left-5 top-1/2 -translate-y-1/2 text-on-surface-variant/20" size={16} />
              <select className="bg-surface-container-low border-4 border-surface-container-lowest rounded-2xl pl-12 pr-10 py-4 text-[10px] font-black focus:outline-none focus:border-primary-container/20 text-on-surface-variant appearance-none cursor-pointer uppercase tracking-[0.2em] shadow-inner w-full">
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y-4 divide-surface-container-low">
              {users?.map((user) => (
                <tr key={user.id} className="hover:bg-primary-container/5 transition-all duration-700 group/row">
                  ...
                  <td className="px-10 py-8">
                    <span className={`text-[10px] px-4 py-1.5 rounded-full font-black border-2 uppercase tracking-[0.2em] italic transition-all duration-700 ${
                      user.role === 'ADMIN' ? 'bg-primary-container/10 text-primary-container border-primary-container/20 shadow-[0_0_15px_rgba(246,139,30,0.2)]' :
                      user.role === 'MODERATOR' ? 'bg-on-surface text-white border-on-surface' :
                  ...
                  <td className="px-10 py-8 text-right">
                    <div className="flex justify-end gap-3">
                      <button 
                        ...
                        className={`p-4 rounded-2xl border-4 duration-500 transition-all cursor-pointer shadow-soft group/btn ${
                          user.isActive 
