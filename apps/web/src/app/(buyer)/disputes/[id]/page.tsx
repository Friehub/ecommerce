...
export default function DisputeThreadPage() {
 ...
 onSuccess: () => {
 utils.dispute.getThread.invalidate({ disputeId: id });
 toast({ title: 'Dispute Escalated', message: 'This case is now under support review.', type: 'info' });
 }
 });
 ...
 <Skeleton className="h-24 w-2/3 rounded" />
 <Skeleton className="h-24 w-2/3 ml-auto rounded" />
 </div>
 ...
 <h2 className="text-3xl font-semibold text-on-surface uppercase tracking-tighter mb-4">Dispute Not Found</h2>
 <Link href="/disputes" className="text-jumia-orange font-semibold uppercase tracking-widest hover:underline">Back to Dispute Center</Link>
 </div>
 );
 }
 ...
 {/* Header / Info */}
 <div className="bg-surface-container-low rounded border border-surface-container-lowest shadow-soft p-8 relative overflow-hidden">
 ...
 <h1 className="text-2xl md:text-3xl font-semibold text-on-surface tracking-tighter leading-none mb-6 uppercase">{dispute.reason}</h1>
 ...
 <User size={14} className="text-jumia-orange" />
 </div>
 <span className="text-[10px] font-semibold uppercase tracking-widest text-on-surface">{dispute.buyer.firstName} {dispute.buyer.lastName}</span>
 </div>
 ...
 <span className="text-[10px] font-semibold uppercase tracking-widest text-on-surface">{dispute.seller.businessName}</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className={`max-w-[85%] rounded p-6 shadow-soft border-2 ${
 isMe 
 ? 'bg-jumia-orange text-white border-transparent rounded-tr-none shadow-on-surface/10' 
 ...
 <form onSubmit={handleSend} className="bg-surface-container-lowest rounded border border-surface-container-low shadow-2xl p-4 flex gap-4 items-end">
 <textarea 
 value={message}
 onChange={(e) => setMessage(e.target.value)}
 placeholder="Elaborate on your case..."
 rows={2}
 className="flex-1 bg-surface-container-low border-2 border-transparent focus:border-jumia-orange/30 focus:bg-surface-container-lowest rounded p-4 text-sm font-medium transition-all outline-none resize-none placeholder:font-normal placeholder:text-on-surface-variant/50"
 />
 <button 
 type="submit"
 disabled={sendMessage.isLoading || !message.trim()}
 className="bg-jumia-orange text-white w-16 h-16 rounded shadow-2xl shadow-primary-container/40 flex items-center justify-center hover:-translate-y-1 hover:shadow-primary-container/60 transition-all active:scale-90 disabled:opacity-50 flex-shrink-0"
 >
 ...
 placeholder="0.00"
 id="refundAmount"
 className="w-full bg-surface-container-lowest border-2 border-outline-variant/30 rounded-2xl p-4 text-sm font-semibold focus:border-jumia-orange outline-none transition-all"
 />
 </div>
