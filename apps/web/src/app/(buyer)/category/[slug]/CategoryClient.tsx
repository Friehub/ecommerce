// apps/web/src/app/(buyer)/category/[slug]/CategoryClient.tsx
'use client';
...
import { ChevronRight, SlidersHorizontal, PackageSearch, Search } from 'lucide-react';
          ...
          <div className="bg-white rounded-sm border border-j-border shadow-sm sticky top-24 overflow-hidden">
            ...
            <div className="p-5 flex flex-col gap-10">
              ...
              <div className="pt-6 border-t border-j-border">
                <h4 className="text-[10px] font-black mb-4 uppercase tracking-widest text-j-text-muted">Price Range (₦)</h4>
                <div className="flex flex-col gap-4">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center text-[10px] font-black text-j-text">
                      <span>₦0</span>
                      <span className="text-jumia-orange bg-orange-50 border border-orange-100 px-2 py-0.5 rounded-sm">Max: ₦{parseInt(maxPrice || '500000').toLocaleString()}</span>
                    </div>
                    <input 
                      type="range"
                      min="1000"
                      ...
                      className="w-full h-1.5 bg-j-border rounded-lg appearance-none cursor-pointer accent-jumia-orange focus:outline-none"
                    />
                  </div>

                  <div className="flex items-center gap-3">
                    <div className="w-full bg-j-background border-2 border-j-border rounded-sm px-3 py-2 text-[11px] font-black text-j-text-muted">
                      Min: ₦0
                    </div>
                    <span className="text-j-text-muted font-black">-</span>
                    <div className="w-full bg-j-background border-2 border-j-border rounded-sm px-3 py-2 text-[11px] font-black text-j-text-muted">
                      Max: ₦{parseInt(maxPrice || '500000').toLocaleString()}
                    </div>
                  </div>
                  <button 
          ...
          {isProdLoading ? (
            ...
            <>
              ...
              {(() => {
                ...
                return (
                  <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-j-border pt-6">
