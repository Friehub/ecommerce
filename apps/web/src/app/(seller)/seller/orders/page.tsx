...
export default function SellerOrdersPage() {
 ...
 <Zap size={24} className="text-jumia-orange" />
 </div>
 <span className="text-[10px] font-semibold uppercase tracking-[0.5em] text-jumia-orange italic">Logistics Hub & Fulfillment Nexus</span>
 </div>
 <h1 className="text-5xl md:text-8xl font-semibold text-on-surface uppercase tracking-tighter leading-[0.85]">
 Operational <br />
 <span className="text-jumia-orange italic">Control.</span>
 </h1>
 <p className="text-on-surface-variant text-[10px] font-semibold uppercase tracking-[0.4em] mt-8 opacity-40 italic border-l-4 border-jumia-orange pl-8">Global Pipeline Orchestration & Shipment Intelligence</p>
 </div>
 ...
 className={`flex items-center gap-4 px-10 py-5 rounded text-[10px] font-semibold uppercase tracking-[0.3em] transition-all duration-500 whitespace-nowrap border relative overflow-hidden group ${
 isActive 
 ? 'bg-jumia-orange text-white border-on-surface shadow-2xl scale-105 -translate-y-2' 
 : 'bg-surface-container-low text-on-surface-variant border-surface-container-lowest hover:border-jumia-orange/20'
 }`}
 ...
 {tab.name}
 {isActive && (
 <div className="absolute top-0 right-0 w-8 h-8 bg-jumia-orange rounded-bl-3xl flex items-center justify-center animate-in slide-in-from-top-4 duration-500">
 ...
 <div className="text-lg font-semibold text-on-surface tracking-tighter uppercase leading-none group-hover:text-jumia-orange transition-colors duration-500">#{pkg.orderId.slice(-12).toUpperCase()}</div>
 </div>
 <div className="text-[9px] font-semibold text-on-surface-variant/30 uppercase tracking-[0.3em] flex items-center gap-3 italic">
 <Clock size={14} className="opacity-40" /> INITIATED {format(new Date(pkg.order.createdAt), 'MMM dd, HH:mm')}
 </div>
 </td>
 <td className="px-12 py-10">
 <div className="flex items-center gap-8">
 <div className="w-20 h-20 bg-surface-container-low rounded flex items-center justify-center border-2 border-outline-variant/5 shrink-0 group-hover:scale-105 group-hover:border-jumia-orange/30 transition-all duration-1000 shadow-inner">
 <Box size={32} className="text-on-surface-variant/10 group-hover:text-jumia-orange transition-colors duration-500" />
 </div>
 <div className="max-w-[340px]">
 <h4 className="font-semibold text-base text-on-surface tracking-tighter uppercase truncate mb-2 leading-none">
 {(pkg.lines[0]?.variant as any)?.product?.title || 'ASSORTED ASSET BUNDLE'}
 </h4>
 {pkg.lines.length > 1 ? (
 <div className="text-[9px] text-jumia-orange font-semibold uppercase tracking-[0.2em] italic bg-jumia-orange/10 px-3 py-1 rounded-xl inline-block border border-jumia-orange/20">+{pkg.lines.length - 1} SUPPLEMENTARY ASSETS</div>
 ) : (
 <div className="text-[9px] text-on-surface-variant/20 font-semibold uppercase tracking-[0.3em] italic">SINGLE LINE ITEM NODE</div>
 )}
 </div>
 </div>
 </td>
 <td className="px-12 py-10">
 <div className="text-2xl font-semibold text-on-surface tracking-tighter leading-none mb-3">₦{Number(pkg.order.total).toLocaleString()}</div>
 <div className="text-[9px] font-semibold text-on-surface-variant/20 uppercase tracking-[0.3em] italic">
 ...
 pkg.status === 'PENDING' ? 'bg-jumia-orange/10 text-jumia-orange border-jumia-orange/20' :
 ...
 className="bg-jumia-orange text-white px-10 py-5 rounded text-[10px] font-semibold uppercase tracking-[0.3em] hover:bg-jumia-orange-dark transition-all shadow-2xl active:scale-95 disabled:opacity-20 flex items-center gap-4 group/btn"
 >
 ...
 disabled={updateStatus.isPending}
 className="bg-jumia-orange text-on-primary-container px-10 py-5 rounded text-[10px] font-semibold uppercase tracking-[0.3em] hover:scale-105 transition-all shadow-2xl active:scale-95 disabled:opacity-20 flex items-center gap-4 group/btn"
 >
 ...
 className="text-[10px] font-semibold bg-surface-container-low border border-surface-container-lowest rounded pl-14 pr-6 py-5 w-48 focus:border-jumia-orange/50 outline-none uppercase tracking-[0.3em] transition-all shadow-inner"
 />
 </div>
 <button 
 ...
 className="bg-success text-white px-10 py-5 rounded text-[10px] font-semibold uppercase tracking-[0.3em] hover:bg-success/90 transition-all shadow-2xl active:scale-95 disabled:opacity-20 flex items-center gap-4 group/btn"
 >
 ...
 <div className="flex items-center gap-4 bg-success-container/10 border border-success/20 px-10 py-5 rounded shadow-soft animate-in zoom-in-95 duration-700">
 <CheckCircle2 size={18} className="text-success animate-pulse" />
 <span className="text-[10px] font-semibold text-success uppercase tracking-[0.4em] italic leading-none">Node Transmitting</span>
 </div>
 )}
 </div>
 </td>
 ...
 <div className="w-32 h-32 bg-surface-container-low rounded-[48px] flex items-center justify-center border border-surface-container-lowest text-on-surface-variant shadow-inner group">
 <ShieldCheck size={64} strokeWidth={1} className="opacity-10 group-hover:scale-110 transition-transform duration-1000" />
 </div>
 <div className="space-y-4">
 <h4 className="font-semibold text-3xl text-on-surface tracking-tighter uppercase leading-none">Pipeline Dormant</h4>
 <p className="text-on-surface-variant text-[10px] font-semibold uppercase tracking-[0.4em] italic leading-relaxed">System awaiting inbound logistics triggers. All current sectors reporting clear fulfillment telemetry.</p>
 </div>
 </div>
 </td>
 </tr>
